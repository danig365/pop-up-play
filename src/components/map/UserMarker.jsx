...
function UserMarker({ profile, isCurrentUser, unreadFromUser = 0 }) {
  ...
  // Touch screens don't have real hover — Leaflet still fires a simulated
  // mouseover right after a tap. Skip it on touch and rely on click/tap
  // (below) to open the popup instead.
  ...
  return (
    <Marker
      ...
      eventHandlers={{
        mouseover: (e) => {
          ...
          // No close-on-mouseout: once open, the popup stays up regardless of
          // where the mouse goes — only the X button, or opening a different
          // marker's popup (Leaflet closes the previous one automatically),
          // closes it.
          e.target.openPopup();
        },
        click: (e) => {
          ...
          // closing is handled only by the in-popup X button or by opening a
    ...
    >
      ...
      <Popup className="custom-popup leaflet-popup" closeButton={false} maxWidth={190} minWidth={160} autoPan={false}>
        {/* Only show popup content for paid users or current user */}
        {(isCurrentUser || hasAccess) ? (
        <div
          className="p-0.5 w-[176px] max-w-[70vw] bg-white rounded-xl relative"
        ...
        >
          <button
            ...
            className="absolute top-0 right-0 w-4 h-4 flex items-center justify-center rounded-full text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors z-10"
            title="Close"
          >
            <X className="w-3 h-3" />
          </button>
          <div className="flex items-center gap-1 mb-0.5 pr-2.5">
            <img
              ...
              className="w-7 h-7 rounded-full object-cover border-2 border-violet-200 flex-shrink-0"
            ...
            <div className="min-w-0">
              ...
              {profile.age && <p className="text-[11px] text-slate-500 leading-tight">{profile.age} years old</p>}
            </div>
          </div>
          {profile.popup_message && (
            <div className="bg-gradient-to-r from-violet-50 to-rose-50 rounded-lg p-1 mt-0.5">
              <p className="text-[11px] text-slate-700 italic leading-tight">"{profile.popup_message}"</p>
            </div>
          )}
          <div className="mt-0.5 space-y-0.5">
            <p className="text-[11px] text-slate-500 font-medium leading-tight truncate">📍 {profile.location || profile.current_city || 'Finding location...'}</p>
            {!isCurrentUser && (
              <div className="flex items-center gap-1">
                <button
                  onClick={handleVideoCall}
                  className="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white rounded-lg px-1.5 py-1 shadow-lg transition-all hover:scale-105 flex items-center justify-center gap-1"
                >
                  <Video className="w-3 h-3" />
                  <span className="text-[11px] font-semibold">Video Verify</span>
                </button>
                <button
                  onClick={handleChatClick}
                  className="bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white rounded-lg p-1 shadow-lg transition-all hover:scale-110 relative"
                >
          ...
          <div className="p-2.5 min-w-[160px] bg-white rounded-xl text-center">
